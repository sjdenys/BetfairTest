...
/**
 * APINGAccountRequester: sends requests to Betfair API asynchronously, receives response, then returns it via a listener
 *
 * @
 * PUBLIC FEATURES:
 * // Constructors
 *    APINGAccountRequester()
 * // Methods
 ...
 * @author Sean Denys
 */
public class APINGAccountRequester {

    private String strHttpResponse;
    private static String strSessionKey = "";
    private static Double dblAusBalance = 0.0;
    private HTTPResponseListener listener;

    public APINGAccountRequester() {
        this.strHttpResponse = null;
    }

    public static String getStrHttpResponse(){
        return APINGAccountRequester.getStrHttpResponse();
    }

    public static void setStrSessionKey(String strSessionKey) {
        APINGAccountRequester.strSessionKey = strSessionKey;
    }

    public static Double getDblAusBalance() {
        return dblAusBalance;
    }

    public static void setDblAusBalance(Double dblAusBalance) {
        APINGAccountRequester.dblAusBalance = dblAusBalance;
    }
    ...
    private class AsyncRequester extends AsyncTask<Object, Void, ArrayList<String>> {
        ...
        public ArrayList<String> performPostCall(String strRequestType, JSONRPCRequest jrrAPIRequest) {
            ...
            try {
                ...
                httpurlcnnctn.setRequestProperty("X-Authentication", APINGAccountRequester.strSessionKey);
                Gson gson = new GsonBuilder().setDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'").create();
                String strRequestString = gson.toJson(jrrAPIRequest);
                Log.d("Request String",strRequestString);
